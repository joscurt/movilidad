...
class Oracle extends DboSource {
    ...
    function connect() {
        ...
        $config['charset'] = !empty($config['charset']) ? $config['charset'] : 'utf8';
        // $config['charset'] = "WE8ISO8859P1";
        $random = rand(1, 2);
        #exit("#".$random."#");
        if ((int) $random === 2)
            $config['database'] = isset($config['database2']) ? $config['database2'] : $config['database'];
        ...
        foreach ($this->_results[$this->_currentRow] as $index => $field) {
            ...
            if (strpos($column, ' count')) {
                ...
                if(is_numeric($table))
                    ...
                    $resultRow[$table][strtoupper($column)] = $this->_results[$this->_currentRow][$index];
                    #debug($resultRow);
                }
    ...
    function listSources($data = null) {
        ...
        # Se deja limitado a "user_views y user_tables" para optimizar consulta y reducir tiempo de ejecusión.
        $sql = 'SELECT view_name AS name FROM user_views UNION SELECT table_name AS name FROM user_tables WHERE owner = \'';
        $sql .= '\'';

        #$sql = 'SELECT view_name AS name FROM all_views UNION SELECT table_name AS name FROM all_tables WHERE owner = \'';
        #$sql .= 'LIBROVIRTUALCLASES\'';
    ...
    public function describe($model) {
        ...
        if(strpos($this->fullTableName($model), '\'') === FALSE){
            $sql .= " AND OWNER = 'MOVILIDAD'"; // cambio owner
        }
        ...
        for ($i = 0; $row = $this->fetchRow(); $i++) {
            $fields[strtoupper($row[0]['COLUMN_NAME'])] = array(
    ...
    function renderStatement($type, $data) {
        ...
        switch (strtolower($type)) {
            ...
            case 'update':
                ...
                #debug("UPDATE {$table} {$aliases}SET {$fields} {$conditions}");
                #exit();
     ...
     * @param array $stack
     */
    function queryAssociation(Model $model, &$linkModel, $type, $association, $assocData, &$queryData, $external, &$resultSet, $recursive, $stack) {
