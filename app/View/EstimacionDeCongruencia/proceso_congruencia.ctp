...
<div class="container margin-top2">
  ...
  <p class="lead"><b>Etapa 1:</b> Cargar mapa académico Duoc UC</p>
  <p class="text-muted">
    <b>Instrucciones: </b>
...
<div class="container">
  <table class="table table-hover">
    ...
    <tbody>
      ...
      ?>
      <tr>
        <td><?php echo $congruencia['Congruencia']['MOV_LICEOS_RBD']; ?></td>
        <td><?php echo $congruencia['Liceos']['NOM_FUNDA_CORPORACION']; ?></td>
        <td><?php echo $congruencia['Liceos']['NOMBRE_ESTABLECIMEINTO']; ?></td>
        <td><?php echo $congruencia['Congruencia']['ESPECIALIDAD_LICEO']; ?></td>
        <td>Mecánica</td>
        <td>Carga mapa duoc</td>
        <td><?php if(isset($congruencia['Usuarios'])){
          echo $congruencia['Usuarios']['NOMBRE'] ." ". $congruencia['Usuarios']['APELLIDO'];
        } ?></td>
        <td><?php if(isset($congruencia['EstadoCongruencia'])){
          echo $congruencia['EstadoCongruencia']['ESTADO_CONGRUENCIA'];
        } ?></td>
        <td class="text-center"><?php $date = date_create($congruencia['Liceos']['FECHA_MODIFICACION']);
          echo date_format($date, 'd/m/Y');
         ?></td>
        <td class="text-center">
          <a href="<?php echo $this->Html->url(array('action'=>'IngresarCarrera',$congruencia['Congruencia']['ID_CONGRUENCIA'])) ?>">
            <i class="fa fa-edit"></i>
          </a>
        </td>
        <td class="text-center">
          <a href="<?php echo $this->Html->url(array('controller' => 'Archivos','action'=>'Lista',$congruencia['Congruencia']['ID_CONGRUENCIA'])) ?>">
